...
function HomeContent() {
  ...
  return (
    <main>
      ...
      {/* ══════════════════════════════════════════════════════════
          MOBILE HERO — completely separate visual identity
          Warm amber-gold companion aesthetic vs desktop analytical
      ══════════════════════════════════════════════════════════ */}
      <div className="md:hidden min-h-[calc(100dvh-64px)] flex flex-col relative bg-[#070509]">

        {/* Warm amber radial glow — mobile only */}
        {!reduced && (
          <motion.div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: znMode
                ? "radial-gradient(ellipse 90% 60% at 50% 38%, rgba(251,191,36,0.22) 0%, rgba(217,119,6,0.12) 45%, transparent 70%)"
                : "radial-gradient(ellipse 90% 60% at 50% 38%, rgba(251,191,36,0.09) 0%, rgba(217,119,6,0.05) 55%, transparent 75%)",
            }}
            animate={
              znMode
                ? { opacity: [0, 1, 0.6, 1, 0.7, 1] }
                : { opacity: [0.6, 1, 0.6] }
            }
            transition={
              znMode
                ? { duration: 5, times: [0, 0.2, 0.4, 0.6, 0.8, 1] }
                : { duration: 6, repeat: Infinity, ease: "easeInOut" }
            }
          />
        )}

        {/* Top bar — email + github icons, hidden in znMode */}
        {!znMode && (
          <div className="absolute top-4 left-0 right-0 flex items-center justify-center gap-4 z-10">
            <a
              ...
              className="text-amber-900/50 hover:text-amber-700/70 transition-colors"
            >
        ...
        <div className="pt-[22vh] flex flex-col items-center text-center px-6">
          ...
          {!znMode && (
            ...
            >
              <Link
                href="/analyze/hybrid"
                className="w-full py-4 rounded-2xl text-sm font-semibold bg-amber-500 text-black text-center flex items-center justify-center gap-2 min-h-[52px] tap-highlight-none active:scale-[0.98] transition-transform"
              >
                <span>✦</span> Self Analysis
              </Link>
              <Link
                href="/analyze/romantic"
                className="w-full py-4 rounded-2xl text-sm font-semibold bg-amber-500/[0.12] text-amber-200 border border-amber-500/25 text-center flex items-center justify-center gap-2 min-h-[52px] tap-highlight-none active:scale-[0.98] transition-transform"
              >
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-1 pb-2">
                      {MORE_OPTIONS.map((opt) => (
                        <Link
                          key={opt.href}
                          href={opt.href}
                          className="flex flex-col items-center gap-1.5 py-3.5 px-2 rounded-xl bg-amber-500/[0.07] border border-amber-500/[0.14] tap-highlight-none active:scale-[0.96] transition-transform"
                        >
      ...
      {/* Zodicognac progress bar — thin amber sweep, above mobile nav */}
      {znMode && !reduced && (
        <motion.div
