...
const INPUT = "rounded-lg bg-zinc-900 border border-white/10 px-3 py-2 text-white text-sm placeholder-zinc-500 focus:outline-none focus:border-white/30 transition-colors";
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="flex gap-2">
        <input className={`${INPUT} flex-1 min-w-0`} placeholder="Name" value={value.name} onChange={set("name")} />
        <input className={`${INPUT} w-16 shrink-0`} placeholder="Day" type="number" min={1} max={31} value={value.day} onChange={set("day")} />
        <input className={`${INPUT} w-16 shrink-0`} placeholder="Mo" type="number" min={1} max={12} value={value.month} onChange={set("month")} />
      </div>
    </div>
  );
}
