...
def build_stream_prompt(analysis_type: str, ctx: dict) -> str:
    ...
    if analysis_type == "emotional_compatibility":
        ...
        return (
            ...
            f"  Expression similarity: {e.get('emotional_expression_similarity', '?')}%\n"
            f"  Intensity alignment: {e.get('emotional_intensity_alignment', '?')}%\n"
            f"  Stability compatibility: {e.get('emotional_stability_compatibility', '?')}%\n\n"
    ...
    if analysis_type == "sextrology_analysis":
        ...
        return (
            ...
            f"  Sexual compatibility score: {s.get('sexual_compatibility_score', '?')}%\n"
            f"  Intimacy intensity alignment: {s.get('intimacy_intensity_alignment', '?')}%\n"
            f"  Intimacy pacing alignment: {s.get('intimacy_pacing_alignment', '?')}%\n"
            f"  Dominance–receptiveness polarity: {s.get('dominance_receptiveness_polarity', '?')}%\n"
            f"  Emotional–physical balance: {s.get('emotional_physical_balance_similarity', '?')}%\n\n"
